...
bool Block::PoW::IsValid(const void *pInput, uint32_t nSizeInput, uint32_t hashVersion) const
{
	...
	return hlp.getCurrentPoW(hashVersion)->IsValidSolution(hlp.m_Blake, v) &&
				 hlp.TestDifficulty(&m_Indices.front(), (uint32_t)m_Indices.size(), m_Difficulty);
}

bool Block::PoW::IsValidSolution(const void *pInput, uint32_t nSizeInput, uint32_t hashVersion) const
{
	Helper hlp;
	hlp.Reset(pInput, nSizeInput, m_Nonce, hashVersion);
...
int verifySolution(const char* input, const char* nonce, const char* output)
{
	...
	return int(pow.IsValidSolution(input, 32, 2));
}
